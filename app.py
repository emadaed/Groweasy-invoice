...
def upload_logo():
    ...
    if file and allowed_file(file.filename):
        filename = secure_filename(f"logo_{int(datetime.now().timestamp())}.{file.filename.rsplit('.', 1)[1].lower()}")
    ...
    try:
        ...
        if logo_path and os.path.exists(logo_path):
            try:
                ...
                pdf.drawInlineImage(logo_path, 50, y - img_height, width=img_width, height=img_height)
                y -= img_height + 10
            except Exception as e:
        ...
        pdf.drawString(width - 150, y, f"INVOICE #: {invoice_data.get('invoice_number', '001')}")
        ...
        pdf.setFont("Helvetica", 10)
        pdf.drawString(50, y - 15, invoice_data.get('vendor_name', 'Your Business Name'))
        pdf.drawString(50, y - 30, invoice_data.get('vendor_address', 'Business Address'))
        ...
        pdf.drawString(300, y - 30, invoice_data.get('customer_address', 'Customer Address'))
        ...
        pdf.drawString(50, 30, "Generated by DigiReceipt - Pakistan's Premier Invoice Solution")
...
def create_interactive_qr(data, logo_path=None):
    ...
    try:
        ...
        qr_img = qr.make_image(
            fill_color=(30, 64, 175),  # Professional blue
            back_color="white"
        ).convert('RGB')
